...
class TreacheryPokemonV2(commands.Cog):
    ...
    async def v2spawn(self, ctx):
        try:
            pokemon_number = random.randint(1, 1050)
            artwork_url = pb.SpriteResource('pokemon', pokemon_number, other=True, official_artwork=True).url
            embed = discord.Embed(title="A wild Pokémon appeared!").set_image(url=artwork_url)
