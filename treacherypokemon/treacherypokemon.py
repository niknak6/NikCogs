...
class TreacheryPokemon(commands.Cog):
    ...
    async def battle(self, ctx, opponent: discord.Member):
        ...
        async def update_battle_state(p1_name, p2_name, p1_id, p2_id):
            combined_image_file = await self.combatsprite(ctx, p1_id, p2_id)
            battle_embed.set_image(url=f"attachment://{combined_image_file.filename}")
            battle_embed.set_field_at(1, name=f"{p1_name} HP", value=f"{round(player1_hp[p1_name])}", inline=True)
            battle_embed.set_field_at(2, name=f"{p2_name} HP", value=f"{round(player2_hp[p2_name])}", inline=True)
            return await ctx.send(embed=battle_embed, file=combined_image_file) if not battle_message else await battle_message.edit(embed=battle_embed, attachments=[combined_image_file])
        ...
        async def process_turn(attacker, defender):
            pokemon, move, type_, move_power = attacker['party'][0], *self.get_random_move(ctx, attacker['party'][0])
            ...
            opposing_types = await fetch_pokemon_type(defender['party'][0])
            ...
            defender['hp'][defender['party'][0]] = max(defender['hp'][defender['party'][0]] - damage, 0)
            formatted_move_name = "No move available" if move == "NULL" else ' '.join(word.capitalize() for word in move.replace('-', ' ').split())
            return f"{attacker['display']}'s {pokemon}: {formatted_move_name} - Damage: {damage} ({multiplier}x)\n"

        battle_embed = discord.Embed(title=f"Battle: {ctx.author.display_name} VS {opponent.display_name}")
        battle_embed.add_field(name="Turn", value=1, inline=False)
        battle_embed.add_field(name=f"{player1_party[0]} HP", value="Loading...", inline=True)
        battle_embed.add_field(name=f"{player2_party[0]} HP", value="Loading...", inline=True)
        ...
        player1_pokemon_id = self.cur.execute('SELECT pokemon_id FROM pokedex WHERE member_id = ? AND pokemon_name = ?', (ctx.author.id, player1_party[0])).fetchone()[0]
        player2_pokemon_id = self.cur.execute('SELECT pokemon_id FROM pokedex WHERE member_id = ? AND pokemon_name = ?', (opponent.id, player2_party[0])).fetchone()[0]

        battle_message = await update_battle_state(player1_party[0], player2_party[0], player1_pokemon_id, player2_pokemon_id)
        ...
        while player1_party and player2_party:
            ...
            for attacker, defender in [
                ({'party': player1_party, 'hp': player1_hp, 'display': ctx.author.display_name},
                {'party': player2_party, 'hp': player2_hp, 'display': opponent.display_name}),
                ({'party': player2_party, 'hp': player2_hp, 'display': opponent.display_name},
                {'party': player1_party, 'hp': player1_hp, 'display': ctx.author.display_name})
            ]:
                if not attacker['party']:
                    continue

                moves_display += await process_turn(attacker, defender)

                if defender['hp'][defender['party'][0]] <= 0:
                    defeated_pokemon.append(f"{defender['party'][0]} ({defender['display']})")
                    defender['party'].pop(0)
                    moves_display += f"{defender['display']}'s {defender['party'][0]} has been defeated!\n"
                    
                    if defender['party']:
                        new_pokemon = defender['party'][0]
                        player1_pokemon_name, player2_pokemon_name = (new_pokemon, player2_party[0]) if defender['display'] == ctx.author.display_name else (player1_party[0], new_pokemon)
                        ...
                        battle_message = await update_battle_state(player1_pokemon_name, player2_pokemon_name, player1_pokemon_id, player2_pokemon_id)
                    else:
                        break
        ...
        winner = "It's a tie!" if not player1_party and not player2_party else (ctx.author.display_name if not player2_party else opponent.display_name)
