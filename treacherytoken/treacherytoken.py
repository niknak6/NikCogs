    ...
    async def wowtoken(self, ctx, region: str = "us"):
        ...
        async with aiohttp.ClientSession() as session: # Use async with to create a session and get the response
            async with session.get(url) as response:
                ...
                data = json.loads(text) # Use json.loads() to parse the string into a json object
        ...
        if data is not None:
            ...
            await ctx.send(f"Sorry, I could not get the WoW token price for {region}. Please try again later.")
