...
class TikTokCog(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.url_patterns = {
            'tiktok': re.compile(r"(https?://)?((\w+)\.)?tiktok.com/\S*"),
            'twitter': re.compile(r"(https?://)?((\w+)\.)?twitter.com/\S*"),
            'x': re.compile(r"(https?://)?((\w+)\.)?x.com/\S*")
        }
        self.new_domains = {
            'tiktok': 'vxtiktok.com/',
    ...
    def construct_new_url(self, url_match, platform):
        return (url_match.group(1) or 'https://') + (url_match.group(2) or '') + '.' + self.new_domains[platform] + url_match.group(3)
